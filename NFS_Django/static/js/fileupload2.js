		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					height          : 36,                 // The height of the browse button
				...
				if (flashInstalled) {
					...
					if (!settings.queueID) {
						...
						$queue.css({"display":"block","position":"fixed","bottom":"0","right":"0","height":"300px","background-color":"#F7F7F7","width":"300px", "border":"1px solid #b9b9b9"});
						$queue.hide();
						$wrapper.after($queue);
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			if ($.inArray('onSelect', settings.overrideEvents) < 0) {
				...
				//$('#' + settings.queueID).show();
				$('#' + settings.queueID).slideDown("500");
		...
		onUploadSuccess : function(file, data, response) {
			...
			if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
				if (data == "exceed") {
					$('#' + file.id).find('.data').html(' - exceeds limit');
					$(".uploadify-queue-item").css("background-color","#FDE5DD");
				} else if (data == "namenotvalid"){
					$('#' + file.id).find('.data').html(' - filename invalid');
					$(".uploadify-queue-item").css("background-color","#FDE5DD");

				} else if (data == "internalerror"){
					$('#' + file.id).find('.data').html(' - internal error');
					$(".uploadify-queue-item").css("background-color","#FDE5DD");

				} else if (data == "exist"){
					$('#' + file.id).find('.data').html(' - filename duplicated');
					$(".uploadify-queue-item").css("background-color","#FDE5DD");
